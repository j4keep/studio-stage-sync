 ...
 * the north-east, a rocky slope in the south-west used for rolling-rock hazards, mud
 * flats scattered through the grass, roaming wildlife, defense-tool pickups, and an
 * abandoned extraction jeep + exit road for the wave-5 "Extraction" objective.
 * Everything is generated from a seed so a future friend-challenge mode can hand every
 * player the same jungle.
 */

export type Terrain = "path" | "grass" | "water" | "shallow" | "mud" | "bridge" | "ruins" | "rock" | "camp" | "road";
...
/** Wildlife that roams the jungle and charges the player if they get too close. */
export type AnimalKind = "boar" | "jaguar" | "croc";
export type AnimalSpot = { x: number; y: number; kind: AnimalKind; roam: number };

/** Defense tools scattered through the jungle — pick one up to fight back. */
export type WeaponKind = "stick" | "repellent" | "flare";
export type WeaponSpot = { x: number; y: number; kind: WeaponKind };
...
export type JungleMap = {
  ...
  weaponSpots: WeaponSpot[];
  /** Abandoned jeep — reach it to start the wave-5 extraction drive. */
  jeep: { x: number; y: number; r: number };
  /** Exit road — drive the jeep here to escape the jungle. */
...
/** Walkable = not open water; bridges over it are fine. */
 ...
 *  a rocky slope, a few mud flats, roaming wildlife, weapon pickups, and an
 *  extraction jeep + exit road. */
export function buildJungle(seed = 20260822): JungleMap {
  ...
  // Roaming wildlife: boars in the grass, jaguars near the ruins, crocs by the river.
  const animalSpots: AnimalSpot[] = [];
  const boarTiles: [number, number][] = [[14, 20], [22, 11], [9, 9], [30, 24]];
  for (const [tx, ty] of boarTiles) {
    if (tiles[idx(tx, ty)] === "water") continue;
    const c = tileCenter(tx, ty);
    animalSpots.push({ x: c.x, y: c.y, kind: "boar", roam: 90 });
  }
  const jaguarTiles: [number, number][] = [[27, 9], [35, 12]];
  for (const [tx, ty] of jaguarTiles) {
    if (tiles[idx(tx, ty)] === "water") continue;
    const c = tileCenter(tx, ty);
    animalSpots.push({ x: c.x, y: c.y, kind: "jaguar", roam: 120 });
  }
  const crocTiles: [number, number][] = [[13, 15], [26, 16]];
  for (const [tx, ty] of crocTiles) {
    const cy = riverBaseY + riverWobble(tx);
    const c = tileCenter(tx, Math.round(cy));
    animalSpots.push({ x: c.x, y: c.y, kind: "croc", roam: 50 });
  }

  // Defense-tool pickups scattered through the jungle.
  const weaponSpots: WeaponSpot[] = [];
  const weaponTiles: [number, number, WeaponKind][] = [
    [8, 12, "stick"],
    [19, 6, "flare"],
    [31, 21, "repellent"],
    [5, 22, "stick"],
  ];
  for (const [tx, ty, kind] of weaponTiles) {
    if (tiles[idx(tx, ty)] === "water") continue;
    const c = tileCenter(tx, ty);
    weaponSpots.push({ x: c.x, y: c.y, kind });
  }

  // Abandoned camp clearing tucked near the rocky shelf, jeep parked in the middle of
  // it, exit road on the far south-east edge.
  const campT = { x: 9, y: 27, r: 2.2 };
  for (let ty = 0; ty < GRID_H; ty++) {
    for (let tx = 0; tx < GRID_W; tx++) {
      const i = idx(tx, ty);
      if (tiles[i] === "water" || tiles[i] === "shallow" || tiles[i] === "bridge") continue;
      if (dist(tx, ty, campT.x, campT.y) <= campT.r) tiles[i] = "camp";
    }
  }
  const jeepC = tileCenter(campT.x, campT.y);

  // Exit road connecting the camp to the far south-east edge of the map.
  const exitT = { x: 37, y: 27 };
  for (let tx = campT.x; tx <= exitT.x; tx++) {
    for (let dy = -1; dy <= 1; dy++) {
      const ty = exitT.y + dy;
      if (ty < 0 || ty >= GRID_H) continue;
      const i = idx(tx, ty);
  ...
  const exitC = tileCenter(exitT.x, exitT.y);
  ...
  return {
    ...
    animalSpots,
    weaponSpots,
    jeep: { x: jeepC.x, y: jeepC.y, r: 44 },
    exit: { x: exitC.x, y: exitC.y, r: 60 },
    bridgeTiles,
  };
}
