/**
 * Procedural Tumble Guys sound effects (WebAudio, no audio files) — a jump chirp, a coin pickup
 * blip, a tumble/crash thud, a finish-line fanfare, and the match-win jingle.
 */

const KEY = "yaj.games.tumbleguys.sfx.muted";
...
class TumbleGuysSfx {
  ...
  /** Runner tumbles — hit an obstacle or missed a jump. */
  tumble() {
    if (this.muted) return;
    const ctx = this.ensure();
    void ctx.resume().catch(() => undefined);
...
export const tumbleGuysSfx = new TumbleGuysSfx();
