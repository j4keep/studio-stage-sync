...
const ProjectCard = ({ project, onRemove }: { project: Project; onRemove: (id: string) => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl bg-card border border-border overflow-hidden group">
      <div className="relative w-full h-36">
        <img src={project.cover_url} alt={project.title} className="w-full h-full object-cover" />
        <button onClick={() => onRemove(project.id)} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div className="p-4">
        ...
        {project.categories.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {project.categories.map(c => <span key={c} className="text-[9px] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium">{c}</span>)}
          </div>
        )}
        ...
        {project.tiers.length > 0 && (
          <div className="mb-3">
            <div className="flex items-center gap-1 mb-1.5"><Gift className="w-3 h-3 text-primary" /><span className="text-[10px] font-semibold text-foreground">Reward Tiers</span></div>
            {project.tiers.map(t => <span key={t} className="block text-[10px] text-muted-foreground pl-4">• {t}</span>)}
          </div>
        )}
        <button className="w-full py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-semibold glow-primary hover:opacity-90 transition-opacity">Contribute</button>
      </div>
    </motion.div>
  );
};
...
const MyProjectsPage = () => {
  ...
  const fetchProjects = async () => {
    const { data, error } = await (supabase as any).from("projects").select("*").eq("user_id", user!.id).order("created_at", { ascending: false });
    if (!error && data) {
      setProjects(data.map((p: any) => ({ id: p.id, title: p.title, description: p.description || "", goal: p.goal || 1000, raised: p.raised || 0, backers: p.backers || 0, deadline: p.deadline || "TBD", cover_url: p.cover_url || artist1, categories: p.categories || [], tiers: p.tiers || [] })));
    }
    setLoading(false);
  };
  ...
  const handleCreate = async () => {
    if (!newTitle.trim()) { toast({ title: "Missing title", description: "Please enter a project title", variant: "destructive" }); return; }
    if (!coverPreview) { toast({ title: "Cover required", description: "Please upload a cover image for your project", variant: "destructive" }); return; }
    if (selectedCategories.length === 0) { toast({ title: "Category required", description: "Select at least one category", variant: "destructive" }); return; }
    if (!user) return;

    const tiers = newTiers.split("\n").map(t => t.trim()).filter(Boolean);
    const { data, error } = await (supabase as any).from("projects").insert({
      user_id: user.id, title: newTitle.trim(), description: newDesc.trim() || "No description provided.",
      goal: parseInt(newGoal) || 1000, raised: 0, backers: 0, deadline: newDeadline.trim() || "TBD",
      cover_url: coverPreview, categories: selectedCategories, tiers,
    }).select().single();

    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); return; }

    setProjects(prev => [{ id: data.id, title: data.title, description: data.description, goal: data.goal, raised: 0, backers: 0, deadline: data.deadline, cover_url: data.cover_url, categories: data.categories, tiers: data.tiers }, ...prev]);
    ...
    toast({ title: "Project created!", description: `"${data.title}" is now live` });
  };
  ...
  if (!user) return <div className="px-4 pt-4 text-center text-muted-foreground text-sm">Please log in to view your projects.</div>;
    ...
    <div className="px-4 pt-4 pb-4">
      ...
      {showCreate && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mb-4 p-4 rounded-xl bg-card border border-dashed border-primary/30">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button onClick={() => coverInputRef.current?.click()} className="w-full h-32 rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 overflow-hidden hover:border-primary/40 transition-colors">
              {coverPreview ? <img src={coverPreview} alt="Cover" className="w-full h-full object-cover" /> : <><ImagePlus className="w-6 h-6 text-muted-foreground" /><span className="text-xs text-muted-foreground">Upload cover image *</span></>}
            </button>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {categoryOptions.map(cat => (
                  <button key={cat} onClick={() => toggleCategory(cat)} className={`text-[10px] px-2.5 py-1 rounded-full border transition-colors ${selectedCategories.includes(cat) ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:border-primary/40"}`}>{cat}</button>
                ))}
              </div>
            </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {projects.map(p => <ProjectCard key={p.id} project={p} onRemove={removeProject} />)}
        </div>
      )}
