...
import { EVENT_CATEGORIES } from "@/components/events/EventFilterSheet";
...
/** Professional Dashboard → Events: create, edit, share and track RSVPs. */
export default function ProEventsDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (list.length) {
      const { data: rsvps } = await (supabase as any)
        .from("event_rsvps")
        .select("event_id")
        .in("event_id", list.map((r) => r.id));
      const c: Record<string, number> = {};
      ((rsvps as { event_id: string }[]) || []).forEach((r) => {
  ...
  const publish = async () => {
    ...
    try {
      ...
      setEndsAt("");
      setCapacity("");
      setCategory(EVENT_CATEGORIES[0].id);
      setFile(null);
      setShowForm(false);
      void load();
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-background pb-32 text-foreground">
      <header className="sticky top-0 z-20 border-b border-border bg-background/95 px-4 pb-3 pt-3 backdrop-blur">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-primary">Professional</p>
            <h1 className="text-lg font-black tracking-tight">My events</h1>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 border-b border-border px-4 py-4">
          ...
          <label className="block space-y-1">
            <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
              Category (used by event filters)
            </span>
            <select
              value={category}
      ...
      <section className="space-y-3 px-4 py-4">
        {loading ? (
          ...
          rows.map((row) => (
            <article key={row.id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
