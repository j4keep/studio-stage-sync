...
import TumbleGuysBoard from "@/components/games/tumble-guys/TumbleGuysBoard";
import { tumbleGuysSfx } from "@/lib/tumble-guys-sfx";
import { useTurnGame } from "@/hooks/use-turn-game";
import { RunResult, Seat, TumbleRunState, applyRunResult, initialTumbleRun } from "@/lib/tumble-guys-run";
import { bumpStats, createMultiplayerGame, createSoloGame, recordMove, updateGameState } from "@/lib/games";
import { gameRoute } from "@/lib/game-routes";

const HOW_TO_PLAY = [
  "Your runner auto-dashes down the course — tap anywhere to jump.",
  "Every hazard is lethal unless you're airborne for the whole width — time your tap to the obstacle, not before it.",
  "Grab coins for bonus points, and reaching the finish line clean pays out a big bonus.",
  "3 runs each, alternating with your opponent — highest total score wins.",
];

export default function TumbleGuysPage() {
  ...
  const [muted, setMuted] = useState(tumbleGuysSfx.muted);
  const [myName, setMyName] = useState("You");
  const [myAvatar, setMyAvatar] = useState<string | null>(null);

  const run: TumbleRunState = (game?.game_state?.tumbleGuys as TumbleRunState) || initialTumbleRun();
  ...
  const { stats, matchups } = useGameRecord("tumble_guys", user?.id, finished);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await bumpStats(user.id, "tumble_guys", outcome);
      await refresh();
    })();
  }, [finished, game?.id, game?.status]);

  const commit = async (state: TumbleRunState, n: number, nextTurnUserId: string | null) => {
    if (!game || !user) return;
    setGame({ ...game, game_state: { tumbleGuys: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await updateGameState(game.id, { game_state: { tumbleGuys: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await refresh();
  };
  ...
  const rematch = async () => {
    if (!user || !game) return;
    try {
      const state = { tumbleGuys: initialTumbleRun(), moveNumber: 0 };
      const g =
        game.mode === "solo"
          ? await createSoloGame("tumble_guys", user.id, state)
          : opponent?.user_id
            ? await createMultiplayerGame("tumble_guys", user.id, opponent.user_id, state)
            : null;
      if (g) {
        statsWritten.current = null;
        navigate(gameRoute("tumble_guys", g.id), { replace: true });
      }
    } catch (e: any) {
  ...
  const challengeOther = async (opponentId: string, name: string) => {
    if (!user) return;
    try {
      const g = await createMultiplayerGame("tumble_guys", user.id, opponentId, { tumbleGuys: initialTumbleRun(), moveNumber: 0 });
  ...
  const shareResult = async () => {
    const draw = run.winnerSeat === null;
    const iWon = run.winnerSeat === mySeat;
    const text = `I just ${draw ? "tied" : iWon ? "won" : "lost"} a game of Tumble Guys on YAJ 🏃`;
    try {
    ...
    <div className="fixed inset-0 z-[100] overflow-hidden bg-black">
      <div className="relative h-full w-full">
        ...
        <GameIntro
          ...
          title="Tumble Guys"
          subtitle={game.mode === "solo" ? "Dash the obstacle course — solo vs Computer" : `Dash the obstacle course — you vs ${opponentName}`}
          ...
          onPlaySolo={() => {
            ...
            void (async () => {
              if (!user) return;
              try {
                const g = await createSoloGame("tumble_guys", user.id, { tumbleGuys: initialTumbleRun(), moveNumber: 0 });
                statsWritten.current = null;
                navigate(gameRoute("tumble_guys", g.id), { replace: true });
              } catch (e: any) {
      ...
      <OpponentPickerSheet
        ...
        title="Challenge to Tumble Guys"
      />
    </div>
  );
}
