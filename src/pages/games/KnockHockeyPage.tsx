...
import { KnockHockeyState, RoundResult, Seat, applyRoundResult, initialKnockHockey } from "@/lib/knock-hockey-run";
...
export default function KnockHockeyPage() {
  ...
  const [muted, setMuted] = useState(knockHockeySfx.muted);
  const [myName, setMyName] = useState("You");
  const [myAvatar, setMyAvatar] = useState<string | null>(null);

  const run: KnockHockeyState = (game?.game_state?.knockHockey as KnockHockeyState) || initialKnockHockey();
  ...
  const { stats, matchups } = useGameRecord("knock_hockey", user?.id, finished);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await bumpStats(user.id, "knock_hockey", outcome);
      await refresh();
    })();
  }, [finished, game?.id, game?.status]);

  const commit = async (state: KnockHockeyState, n: number, nextTurnUserId: string | null) => {
    if (!game || !user) return;
    setGame({ ...game, game_state: { knockHockey: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await updateGameState(game.id, { game_state: { knockHockey: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await refresh();
  };
  ...
  const rematch = async () => {
    if (!user || !game) return;
    try {
      const state = { knockHockey: initialKnockHockey(), moveNumber: 0 };
      const g =
        game.mode === "solo"
          ? await createSoloGame("knock_hockey", user.id, state)
          : opponent?.user_id
            ? await createMultiplayerGame("knock_hockey", user.id, opponent.user_id, state)
            : null;
      if (g) {
        statsWritten.current = null;
        navigate(gameRoute("knock_hockey", g.id), { replace: true });
      }
    } catch (e: any) {
  ...
  const challengeOther = async (opponentId: string, name: string) => {
    if (!user) return;
    try {
      const g = await createMultiplayerGame("knock_hockey", user.id, opponentId, { knockHockey: initialKnockHockey(), moveNumber: 0 });
  ...
  const shareResult = async () => {
    const draw = run.winnerSeat === null;
    const iWon = run.winnerSeat === mySeat;
    const text = `I just ${draw ? "tied" : iWon ? "won" : "lost"} a game of Knock Hockey on YAJ 🏒`;
    try {
  ...
  const resultDetail = finished
    ? `Final score — you ${run.scores[mySeat]} · ${oppLabel} ${run.scores[oppSeat]}  •  Goals ${run.goals[mySeat]}-${run.goals[oppSeat]}`
    : undefined;

  const toggleMute = () => {
    const next = !muted;
    setMuted(next);
    knockHockeySfx.setMuted(next);
  };
        ...
        <GameIntro
          ...
          onPlaySolo={() => {
            ...
            void (async () => {
              if (!user) return;
              try {
                const g = await createSoloGame("knock_hockey", user.id, { knockHockey: initialKnockHockey(), moveNumber: 0 });
                statsWritten.current = null;
                navigate(gameRoute("knock_hockey", g.id), { replace: true });
              } catch (e: any) {
