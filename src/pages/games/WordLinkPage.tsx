...
import WordLinkBoard from "@/components/games/word-link/WordLinkBoard";
import { wordLinkSfx } from "@/lib/word-link-sfx";
import { useTurnGame } from "@/hooks/use-turn-game";
import { RoundResult, Seat, WordLinkState, applyRoundResult, initialWordLink } from "@/lib/word-link-run";
import { bumpStats, createMultiplayerGame, createSoloGame, endGame, recordMove, updateGameState } from "@/lib/games";
import { gameRoute } from "@/lib/game-routes";

const HOW_TO_PLAY = [
  "Drag across the letter wheel to spell a word, then lift your finger to submit it.",
  "Every valid word you find scores points equal to its length — longer words are worth more.",
  "Find every word in the puzzle for a big bonus before the clock runs out.",
...
export default function WordLinkPage() {
  ...
  const [muted, setMuted] = useState(wordLinkSfx.muted);
  const [myName, setMyName] = useState("You");
  const [myAvatar, setMyAvatar] = useState<string | null>(null);

  const run: WordLinkState = (game?.game_state?.wordLink as WordLinkState) || initialWordLink();
  ...
  const { stats, matchups } = useGameRecord("word_link", user?.id, finished);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await bumpStats(user.id, "word_link", outcome);
      await refresh();
    })();
  }, [finished, game?.id, game?.status]);

  const commit = async (state: WordLinkState, n: number, nextTurnUserId: string | null) => {
    if (!game || !user) return;
    setGame({ ...game, game_state: { wordLink: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await updateGameState(game.id, { game_state: { wordLink: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await refresh();
  };
  ...
  const rematch = async () => {
    if (!user || !game) return;
    try {
      const state = { wordLink: initialWordLink(), moveNumber: 0 };
      const g =
        game.mode === "solo"
          ? await createSoloGame("word_link", user.id, state)
          : opponent?.user_id
            ? await createMultiplayerGame("word_link", user.id, opponent.user_id, state)
            : null;
      if (g) {
        statsWritten.current = null;
        navigate(gameRoute("word_link", g.id), { replace: true });
      }
    } catch (e: any) {
  ...
  const challengeOther = async (opponentId: string, name: string) => {
    if (!user) return;
    try {
      const g = await createMultiplayerGame("word_link", user.id, opponentId, { wordLink: initialWordLink(), moveNumber: 0 });
  ...
  const resultDetail = finished
    ? `Final score — you ${run.scores[mySeat]} · ${oppLabel} ${run.scores[oppSeat]}  •  Words ${run.wordsFound[mySeat]}-${run.wordsFound[oppSeat]}`
    : undefined;

  const toggleMute = () => {
    const next = !muted;
    setMuted(next);
    wordLinkSfx.setMuted(next);
  };
        ...
        <GameIntro
          ...
          onPlaySolo={() => {
            ...
            void (async () => {
              if (!user) return;
              try {
                const g = await createSoloGame("word_link", user.id, { wordLink: initialWordLink(), moveNumber: 0 });
                statsWritten.current = null;
                navigate(gameRoute("word_link", g.id), { replace: true });
              } catch (e: any) {
