...
import BingoBoard from "@/components/games/bingo/BingoBoard";
import { bingoSfx } from "@/lib/bingo-sfx";
import { useTurnGame } from "@/hooks/use-turn-game";
import { BingoState, RoundResult, Seat, applyRoundResult, initialBingo } from "@/lib/bingo-run";
import { bumpStats, createMultiplayerGame, createSoloGame, recordMove, updateGameState } from "@/lib/games";
import { gameRoute } from "@/lib/game-routes";

const HOW_TO_PLAY = [
  "Numbers get called out automatically — any that match your card are marked for you, no need to tap them.",
  "The moment you complete a row, column, or diagonal, the glowing BINGO button lights up — tap it fast to claim the round.",
  "The fewer numbers called before you claim, the more points you score. Wait too long and someone else could beat you to it.",
...
export default function BingoPage() {
  ...
  const [muted, setMuted] = useState(bingoSfx.muted);
  const [myName, setMyName] = useState("You");
  const [myAvatar, setMyAvatar] = useState<string | null>(null);

  const run: BingoState = (game?.game_state?.bingo as BingoState) || initialBingo();
  ...
  const { stats, matchups } = useGameRecord("bingo", user?.id, finished);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      await bumpStats(user.id, "bingo", outcome);
      await refresh();
    })();
  }, [finished, game?.id, game?.status]);

  const commit = async (state: BingoState, n: number, nextTurnUserId: string | null) => {
    if (!game || !user) return;
    setGame({ ...game, game_state: { bingo: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await updateGameState(game.id, { game_state: { bingo: state, moveNumber: n }, current_turn_user_id: nextTurnUserId });
    await refresh();
  };
  ...
  const rematch = async () => {
    if (!user || !game) return;
    try {
      const state = { bingo: initialBingo(), moveNumber: 0 };
      const g =
        game.mode === "solo"
          ? await createSoloGame("bingo", user.id, state)
          : opponent?.user_id
            ? await createMultiplayerGame("bingo", user.id, opponent.user_id, state)
            : null;
      if (g) {
        statsWritten.current = null;
        navigate(gameRoute("bingo", g.id), { replace: true });
      }
    } catch (e: any) {
  ...
  const challengeOther = async (opponentId: string, name: string) => {
    if (!user) return;
    try {
      const g = await createMultiplayerGame("bingo", user.id, opponentId, { bingo: initialBingo(), moveNumber: 0 });
  ...
  const shareResult = async () => {
    const draw = run.winnerSeat === null;
    const iWon = run.winnerSeat === mySeat;
    const text = `I just ${draw ? "tied" : iWon ? "won" : "lost"} a game of Bingo on YAJ 🎱`;
    try {
  ...
  const resultDetail = finished
    ? `Final score — you ${run.scores[mySeat]} · ${oppLabel} ${run.scores[oppSeat]}  •  Bingos ${run.bingos[mySeat]}-${run.bingos[oppSeat]}`
    : undefined;

  const toggleMute = () => {
    const next = !muted;
    setMuted(next);
    bingoSfx.setMuted(next);
  };
    ...
    <div className="fixed inset-0 z-[100] overflow-hidden bg-black">
      <div className="relative h-full w-full">
        ...
        <GameIntro
          ...
          title="Bingo"
          subtitle={game.mode === "solo" ? "Race to a line — solo vs Computer" : `Race to a line — you vs ${opponentName}`}
          ...
          onPlaySolo={() => {
            ...
            void (async () => {
              if (!user) return;
              try {
                const g = await createSoloGame("bingo", user.id, { bingo: initialBingo(), moveNumber: 0 });
                statsWritten.current = null;
                navigate(gameRoute("bingo", g.id), { replace: true });
              } catch (e: any) {
