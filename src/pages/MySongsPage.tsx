...
const MySongsPage = () => {
  ...
  const handlePublishToRadio = async (songId: string) => {
    ...
    setSelectedGenre("");
    toast({ title: "Published to Radio! 📻", description: `Song is now live on YAJ Radio under ${selectedGenre}` });
  };
  ...
  const handleRemoveFromRadio = async (songId: string) => {
    ...
    toast({ title: "Removed from Radio", description: "Song is no longer on YAJ Radio" });
  };
                  ...
                  >
                    <div className="mt-1 p-3 rounded-xl bg-primary/5 border border-primary/20">
                      <p className="text-xs font-semibold text-foreground mb-2 flex items-center gap-1.5">
                        <Radio className="w-3.5 h-3.5 text-primary" /> Publish to YAJ Radio
                      </p>
