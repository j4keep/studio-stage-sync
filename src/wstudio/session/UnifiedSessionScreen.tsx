...
function Fader({ value = 0.5, height = 90, onChange }: { value?: number; height?: number; onChange?: (v: number) => void }) {
  ...
  return (
    <div
      className="relative"
      style={{ width: 18, height, cursor: onChange ? "ns-resize" : "default", touchAction: "none", userSelect: "none" }}
      onPointerDown={onPointerDown}
    >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-col items-end gap-0.5">
                    <span
                      ...
                      style={{
                        background: (hasBooking ? phase : demoClock.phase) === "live"
                          ? "rgba(74,222,96,0.15)"
                          : (hasBooking ? phase : demoClock.phase) === "ended"
                            ? "rgba(239,68,68,0.15)"
                            : "rgba(155,155,170,0.1)",
                        color: (hasBooking ? phase : demoClock.phase) === "live"
                          ? C.green
                          : (hasBooking ? phase : demoClock.phase) === "ended"
                            ? C.red
                            : C.dim,
                    ...
                    >
                      ...
                      {(hasBooking ? phase : demoClock.phase) === "ended"
                        ? "Ended"
                        : (hasBooking ? phase : demoClock.phase) === "live"
                          ? (hasBooking ? timerRunning : demoClock.running) ? "Live" : "Paused"
                          : "Scheduled"}
                    </span>
                    {isEngineer && (
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {isEngineer && hasBooking && phase === "scheduled" && (
                    <button
                      ...
                      style={{ background: "rgba(74,222,96,0.12)", border: `1px solid rgba(74,222,96,0.3)`, color: C.green }}
                    >
                      ▶ Start Timer
                    </button>
                  )}
                  {isEngineer && phase === "ended" && (
                    <button
                      ...
                      className="flex-1 rounded-md px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide"
                      style={{ background: "rgba(245,200,66,0.1)", border: `1px solid rgba(245,200,66,0.3)`, color: C.yellow }}
                    >
                      + Continue (+5m)
                    </button>
                  )}
                    ...
                    <>
                      {([15, 30, 60] as const).map((m) => (
                        <button
                          ...
                          style={{ background: C.inset, border: `1px solid ${C.insetBorder}`, color: C.yellow }}
                        >
                          +{m}m
                        </button>
                      ))}
