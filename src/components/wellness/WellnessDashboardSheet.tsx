...
export default function WellnessDashboardSheet({ open, onClose, onStateChange }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col justify-end bg-black/40 backdrop-blur-[2px]">
      <button type="button" className="min-h-0 flex-1" aria-label="Dismiss" onClick={onClose} />
      <div className="max-h-[88vh] overflow-y-auto rounded-t-[1.75rem] bg-[#f3f7f5] px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl">
        ...
        <button
          type="button"
          onClick={() => save()}
          className="mb-2 h-12 w-full rounded-full bg-teal-700 text-sm font-black text-white shadow-md"
        >
