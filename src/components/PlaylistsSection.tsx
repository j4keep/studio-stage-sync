                        ...
                        <div className="flex flex-col gap-1 mt-2">
                          {playlist.items.map((item, i) => (
                            <div key={item.id} onClick={() => handlePlayItem(item)} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-primary/5 transition-all group cursor-pointer">
                              ...
                              <button onClick={() => removeItemFromPlaylist(playlist.id, item.id)} className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-all"><X className="w-3 h-3" /></button>
                            </div>
                          ))}
                        </div>
                      ) : (
