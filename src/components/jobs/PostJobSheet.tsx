      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-40">
        ...
        <Field label="Description *">
          <textarea value={form.description} onChange={(e) => update("description", e.target.value)}
            rows={5} placeholder="Role overview, responsibilities, must-haves…" className={inputCls} />
        </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Salary Min ($/yr)">
            <input inputMode="numeric" value={form.salary_min} onChange={(e) => update("salary_min", e.target.value)} className={inputCls} />
          </Field>
          <Field label="Salary Max ($/yr)">
            <input inputMode="numeric" value={form.salary_max} onChange={(e) => update("salary_max", e.target.value)} className={inputCls} />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Work mode">
            <select value={form.remote_mode} onChange={(e) => update("remote_mode", e.target.value)} className={inputCls}>
              {REMOTE_MODES.map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
            </select>
          </Field>
          <Field label="Experience">
            <select value={form.experience_level} onChange={(e) => update("experience_level", e.target.value)} className={inputCls}>
        ...
        <Field label="Required Skills (comma-separated)">
          <input value={form.skills} onChange={(e) => update("skills", e.target.value)}
            placeholder="Figma, TypeScript, Design Systems" className={inputCls} />
        </Field>

        <Field label="Education">
          <input value={form.education} onChange={(e) => update("education", e.target.value)}
            placeholder="Bachelor's degree or equivalent" className={inputCls} />
        </Field>

        <Field label="Benefits (comma-separated)">
          <input value={form.benefits} onChange={(e) => update("benefits", e.target.value)}
