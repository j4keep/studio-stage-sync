...
const musicBundle = await createFeedMusicStream(
  selectedMusic,
  musicPreviewUrl,
);
...
try {
  ...
  const recordingBundle = await buildRecordingStream(stream);

  recordingCleanupRef.current = recordingBundle.cleanup;

  const rec = createVideoRecorder(recordingBundle.stream, mime);
...
return ( <div className="absolute inset-0 bg-black flex flex-col touch-none">
{!denied && (
<video
ref={videoRef}
