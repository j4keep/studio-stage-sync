      ...
      <div className="flex items-start justify-between gap-2 p-3">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="flex flex-col items-end gap-1">
            {st.powers.shield > 0 && (
              <span className="rounded-full bg-sky-500/25 px-2 py-0.5 text-[10px] font-black text-sky-100">
                Shield {Math.ceil(st.powers.shield)}s
              </span>
            )}
            {st.powers.speed > 0 && (
              <span className="rounded-full bg-rose-500/25 px-2 py-0.5 text-[10px] font-black text-rose-100">
                Speed {Math.ceil(st.powers.speed)}s
              </span>
            )}
          </div>
        </div>
      ...
      {st.toast && (
        <div className="absolute inset-x-0 bottom-40 flex justify-center">
          <p className="flex items-center gap-1.5 rounded-full border border-white/20 bg-black/60 px-3 py-1.5 text-xs font-black text-white backdrop-blur">
            {safeZoneActive(st) && <Waves className="h-3.5 w-3.5 text-sky-300" />}
            {st.toast.text}
          </p>
        </div>
      )}

      {/* drowning warning ring */}
      {st.submerged > 0.4 && (
        <div className="absolute inset-0 border-[6px] border-sky-400/50" style={{ opacity: st.submerged / 1.7 }} />
      )}
    </div>
  );
}
