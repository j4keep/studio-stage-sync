...
import { NInput } from "@/lib/neighborhood/engine";

type Props = {
  /** Mutable input the engine reads each frame — no React state in the hot path. */
  input: React.MutableRefObject<NInput>;
};
...
/**
 * Free 360° movement stick, same touch-anywhere feel as the other YAJ Adventures: press
 * anywhere and drag to steer the character. WASD / arrows work on desktop. Sits at z-10, below
 * the z-20 HUD, so it never shadows the back/mute/interact buttons — the exact bug fixed on
 * Survival Island's controls layer, built correctly here from the start.
 */
export default function NeighborhoodControls({ input }: Props) {
