-- Interest / approve flow: gig stays open until host approves a helper.
...
-- Helpers no longer self-claim; host approves via poster update
DROP POLICY IF EXISTS "gig_listings claim worker" ON public.gig_listings;

-- Keep open gigs visible publicly (assigned/completed only for parties)
