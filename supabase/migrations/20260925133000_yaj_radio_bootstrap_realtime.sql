-- YAJ Radio bootstrap/hotfix.
-- Safe to run even if the earlier radio station migration was already applied.
...
create index if not exists radio_station_shows_station_idx on public.radio_station_shows(station_id, scheduled_at);
...
begin
  ...
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'radio_station_shows'
  ) then
    alter publication supabase_realtime add table public.radio_station_shows;
  end if;
end
$$;
